    ...
    public function reedit_pr(){
        ...
        if (1)//这个地方可以填写上传文件的限制，比如格式大小要求等，为了方便测试，这里没有写上传限制。
        {
            ...
            else
            {
                ...
                if (!is_dir(base_url()."application/controllers/upload/".$_SESSION["user_id"]))//当路径不穿在
                {
                    mkdir(base_url()."application/controllers/upload/".$_SESSION["user_id"],0777);//创建路径
                }
                $url=base_url()."application/controllers/upload/".$_SESSION["user_id"]."/";//记录路径
